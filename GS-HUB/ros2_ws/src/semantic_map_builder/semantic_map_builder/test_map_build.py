...
class BuildMapTester(Node):
    def __init__(self):
        super().__init__('build_map_tester')
        
        # 创建两个客户端，分别对应 start 和 end 服务
        self._start_client = self.create_client(StartMapBuilder, '/gs_hub/start_build_db')
        self._end_client = self.create_client(EndMapBuilder, '/gs_hub/end_build_db')
    ...
    def call_start_service(self, collection_name: str, dimension: int):
        ...
        request = StartMapBuilder.Request()
        request.collection_name.data = collection_name
    ...
    def call_end_service(self):
        """调用 EndMapBuilder 服务"""
        request = EndMapBuilder.Request()  # 如果有请求字段也在此进行赋值
